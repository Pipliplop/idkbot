...
# Only this user can pull the anonymous message log
OWNER_ID = 1399047867766673609

# Track recent active users per channel for timeout targeting
recent_active_users: dict[int, list[int]] = {}

# --- Anonymous message log, persisted to a GitHub Gist (free, survives Railway redeploys) ---
GITHUB_TOKEN = os.environ.get("GITHUB_TOKEN")
GIST_ID = os.environ.get("GIST_ID")  # optional — if unset, bot creates one and logs the ID to print
GIST_FILENAME = "anon_message_log.json"
...
async def load_anon_log() -> list[dict]:
    """Load the anon message log from the Gist, creating the Gist if GIST_ID isn't set yet."""
    global GIST_ID

    if not GITHUB_TOKEN:
        print("[ANON LOG] No GITHUB_TOKEN set — log will not persist across restarts.")
        return []

    async with aiohttp.ClientSession() as session:
        if not GIST_ID:
            payload = {
                "description": "Moon Man anonymous message log",
                "public": False,
                "files": {GIST_FILENAME: {"content": "[]"}},
            }
            async with session.post(GIST_API_BASE, headers=_gist_headers(), json=payload) as resp:
                if resp.status not in (200, 201):
                    print(f"[ANON LOG] Failed to create gist: {resp.status} {await resp.text()}")
                    return []
                data = await resp.json()
                GIST_ID = data["id"]
                print(
                    f"[ANON LOG] Created new gist for log storage. "
                    f"Set GIST_ID={GIST_ID} in Railway env vars so it's reused on the next deploy."
                )
                return []

        async with session.get(f"{GIST_API_BASE}/{GIST_ID}", headers=_gist_headers()) as resp:
            if resp.status != 200:
                print(f"[ANON LOG] Failed to load gist: {resp.status} {await resp.text()}")
                return []
            data = await resp.json()
...
async def save_anon_log(log: list[dict]) -> None:
    if not GITHUB_TOKEN or not GIST_ID:
        print("[ANON LOG] Cannot save — missing GITHUB_TOKEN or GIST_ID.")
        return

    payload = {"files": {GIST_FILENAME: {"content": json.dumps(log, indent=2)}}}
    async with aiohttp.ClientSession() as session:
        async with session.patch(f"{GIST_API_BASE}/{GIST_ID}", headers=_gist_headers(), json=payload) as resp:
            if resp.status != 200:
                print(f"[ANON LOG] Failed to save gist: {resp.status} {await resp.text()}")


anon_message_log: list[dict] = []
...
@bot.event
async def on_ready():
    global anon_message_log
    print(f"Moon Man is online as {bot.user}")
    anon_message_log = await load_anon_log()
    await tree.sync()
...
@tree.command(name="moonmessage", description="Send someone an anonymous message, delivered by the Moon Man.")
@app_commands.describe(
    target="Who should receive this anonymous message?",
    message="What do you want to say?"
)
async def moonmessage(interaction: discord.Interaction, target: discord.Member, message: str):
    if target.bot:
        await interaction.response.send_message(
            "the moon does not deliver messages to machines.", ephemeral=True
        )
        return
    if target.id == interaction.user.id:
        await interaction.response.send_message(
            "the moon will not deliver a message to yourself. talk to a mirror instead.", ephemeral=True
        )
        return

    dm_text = (
        "🌙 **an anonymous message has arrived via the moon man.**\n\n"
        f"{message}\n\n"
        "*the moon man does not reveal his messengers.*"
    )
    ...
    try:
        ...
        await interaction.response.send_message(
            "the moon tried to reach them but their DMs are closed. the message was not delivered.",
            ephemeral=True
        )
        return
    except discord.HTTPException as e:
        await interaction.response.send_message(
            f"the moon hit turbulence and could not deliver the message. ({e})",
            ephemeral=True
        )
        return
    ...
    anon_message_log.append(entry)
    await save_anon_log(anon_message_log)

    await interaction.response.send_message(
        "the moon man has delivered your message into the void. they will never know it was you.",
        ephemeral=True
    )


@tree.command(name="moonlog", description="[Owner only] View the anonymous message log.")
@app_commands.describe(count="How many recent entries to show (default 10, max 25)")
async def moonlog(interaction: discord.Interaction, count: int = 10):
    if interaction.user.id != OWNER_ID:
        await interaction.response.send_message(
            "the moon does not show its secrets to you.", ephemeral=True
        )
        return

    if not anon_message_log:
        await interaction.response.send_message(
            "the log is empty. no messages have been sent.", ephemeral=True
        )
        return

    count = max(1, min(count, 25))
    recent = anon_message_log[-count:]

    lines = []
    for entry in reversed(recent):
        lines.append(
            f"**{entry['timestamp']}**\n"
            f"From: {entry['sender_name']} (`{entry['sender_id']}`)\n"
            f"To: {entry['target_name']} (`{entry['target_id']}`)\n"
            f"Message: {entry['message']}\n"
        )

    output = "\n".join(lines)
    header = f"🌙 **anonymous message log ({len(recent)} of {len(anon_message_log)} total):**\n\n"

    if len(header) + len(output) <= 1900:
        await interaction.response.send_message(header + output, ephemeral=True)
    else:
        export_path = "anon_log_export.txt"
        with open(export_path, "w") as f:
            f.write(output)
        await interaction.response.send_message(
            "log too long for a message, sending as a file:",
...
async def perform_random_timeout():
    """Times out a single random active user for 1–30 seconds."""
    channel_id = random.choice(RANDOM_MESSAGE_CHANNELS)
    channel = bot.get_channel(channel_id)
    if not channel:
        return
...
async def random_timeout():
    """
    Runs once per day and fires at most two timeouts, one randomly placed in
    each 12-hour half of the day, so they stay spaced apart instead of
    clustering.
    """
    await asyncio.sleep(random.randint(0, 12 * 3600 - 1))
    await perform_random_timeout()
